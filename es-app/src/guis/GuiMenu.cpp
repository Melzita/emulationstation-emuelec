...
GuiMenu::GuiMenu(Window *window, bool animate) : GuiComponent(window), mMenu(window, _("MAIN MENU").c_str()), mVersion(window)
{
	...
	if (isFullUI)
	{
		...
		addEntry(_("UNE SETTINGS").c_str(), true, [this] { openEmuELECSettings(); }, "iconEmuelec"); /* < emuelec */
...
{
	auto s = new GuiSettings(mWindow, "UNE Settings");
   ...
   for(std::stringstream ss(getShOutput(R"(/usr/bin/emuelec-utils resolutions)")); getline(ss, a, ','); ) {
		...
		if (emuelec_video_mode->getSelected() != "-- AUTO-DETECTED RESOLUTIONS --") { 
			...
			std::string msg = _("You are about to set UNE resolution to:") +"\n" + selectedVideoMode + "\n";
 ...
 auto setOverclock = [emuelec_oga_overclock](const std::string& value)
        ...
        if (emuelec_oga_overclock && emuelec_oga_overclock->changed())
        {
            if (selectedoc != "Off")
            {
                ...
                msg += _("In case you cannot boot anymore, create an empty file called \"no_oc.oga\" on the boot (UNE) partition.\n\n");
...
void GuiMenu::openSystemInformations_batocera()
{
	...
	auto texto2 = std::make_shared<TextComponent>(window, _("WWW.UNIVERSOELECTRONICO.COM.MX"), font, color);
	informationsGui->addWithLabel(_("By"), texto2);
...
void GuiMenu::openDeveloperSettings()
{
	...
	s->addWithLabel(_("SWITCH A/B BUTTONS IN UNE"), invertJoy);
	...
	s->addWithLabel(_("CONTROL UNE ONLY WITH FIRST JOYSTICK"), invertJoy);
...
void GuiMenu::openSystemSettings_batocera() 
{
	...
	s->addSaveFunc([overclock_choice, window, language_choice, language, optionsStorage, selectedStorage, s] 
	{
		...
		{	
			...
			std::string msg = _("You are about to set UNE Language to:") +"\n" +  selectedLanguage + "\n";
			msg += _("UNE will restart")+"\n";
			msg += _("Do you want to proceed ?");
...
#define BATOCERA_MANUAL_FILE "/usr/share/batocera/doc/notice.pdf"
...
#ifdef _ENABLEEMUELEC
	s->addEntry(_("RESTART ARCADE BOX"), false, [window] {
		window->pushGui(new GuiMsgBox(window, _("REALLY RESTART ARCADE BOX?"), _("YES"),
			[] {
	...
	s->addEntry(_("REBOOT TO ANDROID TV"), false, [window] {
		window->pushGui(new GuiMsgBox(window, _("REALLY REBOOT TO ANDROID TV?"), _("YES"),
			[] {
	...
	{
		s->addEntry(_("QUIT UNE"), false, [window] {
